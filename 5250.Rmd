...
frax_risk_train <- impute[train_ind,]  %>% select(-c(all_of(fracture_variables),all_of(individual_variables)))
frax_risk_test<- impute[-train_ind,]  %>% select(-c(all_of(fracture_variables),all_of(individual_variables)))
...
xgbmale_train <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_men_train)),ncol = ncol(frax_risk_men_train), byrow = FALSE ) ,label = train$fracture[which(train$RIAGENDR==1)]) 
xgbfemale_train <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_women_train)),ncol = ncol(frax_risk_women_train), byrow = FALSE ) ,label = train$fracture[which(train$RIAGENDR==2)]) 
xgbmale_test <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_men_test)),ncol = ncol(frax_risk_men_test), byrow = FALSE ) ,label = test$fracture[which(test$RIAGENDR==1)]) 
xgbfemale_test <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_women_test)),ncol = ncol(frax_risk_women_test), byrow = FALSE ) ,label = test$fracture[which(test$RIAGENDR==2)]) 

xgb_train <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_train)),ncol = ncol(frax_risk_train), byrow = FALSE ) ,label = train$fracture) 
xgb_test <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_test)),ncol = ncol(frax_risk_test), byrow = FALSE ) ,label = test$fracture) 
...
for(i in seq_len(nrow(hyper_grid))) {
  ...
  m1 <- xgb.train(data = xgb_train,nrounds = 2,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],
 ...
 m3 <- xgb.train(data = xgbfemale_train,nrounds = 2,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],
  ...
  pred_whole<- predict(m1, xgb_train)
  pred_male<- predict(m2, xgbmale_train)
  pred_female <- predict(m3, xgbfemale_train)
  pred_whole <- as.numeric(pred_whole > sort( pred_whole, decreasing = T)[sum(train$fracture)])
  ...
  hyper_grid$accuracy_whole[i]  <- mean(pred_whole == train$fracture)
...
arrange(hyper_grid,accuracy_whole);arrange(hyper_grid,accuracy_male);arrange(hyper_grid,accuracy_female)
bst_whole <- xgb.train(data = xgb_test,nrounds = 2,objective = "binary:logistic",
...
xgb.plot.importance(xgb.importance(names(frax_risk_test),model = bst_whole))
xgb.plot.importance(xgb.importance(names(frax_risk_men_test),model = bst_male))
roc(test$fracture ~ predict(bst_whole, xgb_test))$auc
roc(test$fracture[which(test$RIAGENDR==1)] ~ predict(bst_male , xgbmale_test))$auc
roc(test$fracture[which(test$RIAGENDR==2)] ~ predict(bst_female , xgbfemale_test))$auc
######## DXXWDBMD is the best predictor of male osteoporotic  fracture
xgb.plot.importance(xgb.importance(names(frax_risk_women_test),model = bst_female))
...
for(i in seq_len(nrow(hyper_grid))) {
  set.seed(123) # for reproducibility
  m1 <- xgb.train(data = xgbhip,nrounds = 3,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],subsample=.7,
                    eta = hyper_grid$eta[i],min_child_weight = hyper_grid$min_child_weight[i])
  ...
  pred_hip<- as.numeric(pred_hip > sort( pred_hip, decreasing = T))
...
bst_hip <- xgb.train(data = xgbhip,nrounds = 3,objective ="binary:logistic",max.depth=8,
                    eta = 0.9,min_child_weight =6)
roc(y.hip ~ predict(bst_hip , xgbhip))$auc
bst_whole <- xgb.train(data = xgbfrac_test,nrounds = 3,objective ="binary:logistic",max.depth=4,
                    eta = 0.2,min_child_weight =4)
roc(y.hip[train_ind] ~ predict(bst_whole , xgbfrac_train))$auc
...
library(hrbrthemes)
p5 <- xgb.importance(names(frax_risk_test),model = bst_whole) %>%
      mutate(Feature = fct_reorder(Feature, Gain)) %>%
      ggplot( aes(y = Gain, x = Feature)) +
      geom_bar(stat="identity", fill="#f68060", alpha=.6, width=.4) +
      coord_flip() +
      ggtitle("Whole") + ylab("")+xlab("")+
      theme_bw()+
      theme(plot.title = element_text(hjust = 0.5,size = 10))
p7 <- xgb.importance(names(frax_risk_men_test),model = bst_male) %>%
      mutate(Feature = fct_reorder(Feature, Gain)) %>%
      ggplot( aes(y = Gain, x = Feature)) +
      geom_bar(stat="identity", fill="#f68060", alpha=.6, width=.4) +
      coord_flip() +
      ggtitle("Men") + ylab("")+xlab("")+
      theme_bw()+
      theme(plot.title = element_text(hjust = 0.5,size = 10))
p8 <- xgb.importance(names(frax_risk_women_test),model = bst_female) %>%
      mutate(Feature = fct_reorder(Feature, Gain)) %>%
      ggplot( aes(y = Gain, x = Feature)) +
      geom_bar(stat="identity", fill="#f68060", alpha=.6, width=.4) +
      coord_flip() +
      ggtitle("Women") + ylab("")+xlab("")+
      theme_bw()+
      theme(plot.title = element_text(hjust = 0.5,size = 10))

p6 <- xgb.importance(names(x.risk), model = bst_hip)[-c(4:8,11),] %>%
      mutate(Feature = fct_reorder(Feature, Gain)) %>%
      ggplot( aes(y = Gain, x = Feature)) +
      geom_bar(stat="identity", fill="#f68060", alpha=.6, width=.4) +
      coord_flip() +
      ggtitle("Hip") + ylab("")+xlab("")+
      theme_bw()+
      theme(plot.title = element_text(hjust = 0.5,size = 10))

library(cowplot)
ggdraw() +
  draw_plot(p5, x = 0, y = .5, width = .5, height = .5) +
  draw_plot(p6, x = .5, y = .5, width = .5, height = .5) +
  draw_plot(p7, x = 0, y = 0, width = .5, height = 0.5) +
  draw_plot(p8, x = 0.5, y = 0, width = .5, height = 0.5)
...
BMDmale <- xgb.importance(names(frax_risk_men_train),model = bst_male)
BMDfemale <- xgb.importance(names(frax_risk_men_train),model = bst_female)
