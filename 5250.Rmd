...
post.f <- function(reg.formula, d, set.seed, prior){
  set.seed(set.seed)
  post <- stan_glm(reg.formula, data = d,
...
bayesian.f <- function(outcome.var, d, set.seed, features){
  ...
  prior <- normal(0,2.5)
  # 1. baseline model
  reg.formula <- formula(paste(outcome.var," ~ ", paste(features, collapse = " + ")))
  elpd_null <- post.f(reg.formula=reg.formula, d=d, set.seed=set.seed, prior=prior)$elpd
  while(k>0){
    post <- NULL
    elpd <- NULL
    for (i in 1:length(features)){
      reg.formula <- formula(paste(outcome.var," ~ ", paste(features[-i], collapse = " + ")))
      fit <- post.f(reg.formula, d, set.seed, prior)
      post[[i]] <- fit$post
      elpd[i] <- fit$elpd
    }
    if (max(elpd) > elpd_null){
      k <- max(elpd)-elpd_null
      features <- features[-(which.max(elpd))]
      post_null <- post[[which.max(elpd)]]
      elpd_null <- elpd[which.max(elpd)]
    }
    else {
      k <- 0
    }
  ...
  for (j in 1:length(BMD_variables)){
    reg.formula <- formula(paste(outcome.var," ~ ", 
                                 paste(c(features,BMD_variables[j]), collapse = " + ")))
    fit <- post.f(reg.formula, d, set.seed, prior)
    post.new[[j]] <- fit$post
    elpd.new[j] <- fit$elpd
  }
  if (max(elpd.new) > elpd_null){
...
# selected features
BMD <- whole.fracture$BMD
features <- whole.fracture$features
# performance
d <- test.b
auc <- performance.f(whole.fracture, d)$auc
accurary <- performance.f(whole.fracture, d)$auc
...
men.fracture <- bayesian.f(outcome.var, d, set.seed)
# selected features
BMD <- men.fracture$BMD
features <- men.fracture$features
# performance
d <- test.b[which(test.b$RIAGENDR=="1"),-1]
auc <- performance.f(men.fracture, d)$auc
accurary <- performance.f(men.fracture, d)$auc
# Area under the curve: 0.6072
# accurary :0.6
# ** women ----
d <- train.b[which(train.b$RIAGENDR=="2"),-1]
features <- c(nonBMD_variables,individual_variables[-1])
women.fracture <- bayesian.f(outcome.var, d, set.seed)
# selected features
BMD <- women.fracture$BMD
features <- women.fracture$features
# performance
d <- test.b[which(test.b$RIAGENDR=="2"),-1]
auc <- performance.f(women.fracture, d)$auc
accurary <- performance.f(women.fracture, d)$auc
...
#q1
data.q1<-impute[,!names(impute) %in% c("OSQ010A","OSQ010B","OSQ010C")]
data.q1.tr<-train[,!names(train) %in% c("OSQ010A","OSQ010B","OSQ010C")]
data.q1.test<-test[,!names(test) %in% c("OSQ010A","OSQ010B","OSQ010C")]
glmnet.err.q1 <- NULL

# find the error
set.seed(1234)
x1.tr <-scale(model.matrix(fracture~., data.q1.tr)[,-1])
y1.tr <- data.q1.tr$fracture
cv.l1.tr <- cv.glmnet(x1.tr,y1.tr,family="binomial",nfolds = 5,alpha=1,penalty.factor =c(rep(0,6),rep(1,10),rep(0,12)))
best_lambda.l1<-cv.l1.tr$lambda.min
x1.test<-scale(model.matrix(fracture~., data.q1.test)[,-1])
prob1<-predict(cv.l1.tr,x1.test,s=best_lambda.l1,type="response")
preds1<- ifelse(as.numeric(prob1) > quantile(prob1,probs = threshold.fracture), 1, 0)
# confusion matrix
t1 <- table(data.q1.test$fracture,preds1)
# create vector of misclassification rate
temp.err1<- (t1[1,2]+t1[2,1])/sum(t1)
glmnet.err.q1  <- rbind(glmnet.err.q1,temp.err1)


# find BMD
set.seed(1234)
x1 <-scale(model.matrix(fracture~.,data.q1)[,-1])
y1 <- data.q1$fracture
...
BMDfeature1
## "DXXWDBMD" "DXXL1BMD" "DXXL3BMD"

#q2
data.q2<-impute[,!names(impute) %in% c("fracture","OSQ010B","OSQ010C")]
data.q2.tr<-train[,!names(train) %in% c("fracture","OSQ010B","OSQ010C")]
data.q2.test<-test[,!names(test) %in% c("fracture","OSQ010B","OSQ010C")]
glmnet.err.q2 <- NULL

# find the error
set.seed(1234)
x2.tr <-scale(model.matrix(OSQ010A~., data.q2.tr)[,-1])
y2.tr <- data.q2.tr$OSQ010A
cv.l2.tr <- cv.glmnet(x2.tr,y2.tr,family="binomial",nfolds = 5,alpha=1,penalty.factor =c(rep(0,6),rep(1,10),rep(0,12)))
best_lambda.l2<-cv.l2.tr$lambda.min
x2.test<-scale(model.matrix(OSQ010A~., data.q2.test)[,-1])
prob2<-predict(cv.l2.tr,x2.test,s=best_lambda.l2,type="response")
preds2<- ifelse(as.numeric(prob2) > quantile(prob2,probs = threshold.hip), 1, 0)
# confusion matrix
t2 <- table(data.q2.test$OSQ010A,preds2)
# create vector of misclassification rate
temp.err2<- (t2[1,2]+t2[2,1])/sum(t2) ## cannot calcualte the error because only 1 in prediction
glmnet.err.q2  <- rbind(glmnet.err.q2,temp.err2)


# find BMD
x.hip <-scale(model.matrix(OSQ010A~., data.q2)[,-1])
y.hip <- data.q2$OSQ010A
set.seed(1234)
...
BMDfeature.hip
##"DXXINBMD" "DXXWDBMD" "DXXL3BMD"

#q3
## men
data.q3.men<-frax_risk_men[,!names(frax_risk_men) %in% c("RIAGENDR","OSQ010A","OSQ010B","OSQ010C")]
glmnet.err.q3.men <- NULL
train.men<-train%>%filter(RIAGENDR==1)
test.men<-test%>%filter(RIAGENDR==1)
data.q3.men.train<-train.men[,!names(train.men) %in% c("RIAGENDR","OSQ010A","OSQ010B","OSQ010C")]
data.q3.men.test<-test.men[,!names(test.men) %in% c("RIAGENDR","OSQ010A","OSQ010B","OSQ010C")]
# find the error
set.seed(1234)
x3.tr <-scale(model.matrix(fracture~., data.q3.men.train)[,-1])
y3.tr <- data.q3.men.train$fracture
cv.l3.tr <- cv.glmnet(x3.tr,y3.tr,family="binomial",nfolds = 5,alpha=1,penalty.factor =c(rep(0,5),rep(1,10),rep(0,12)))
best_lambda.l3<-cv.l3.tr$lambda.min
x3.test<-scale(model.matrix(fracture~., data.q3.men.test)[,-1])
prob3<-predict(cv.l3.tr,x3.test,s=best_lambda.l3,type="response")
preds3<- ifelse(as.numeric(prob3) > quantile(prob3,probs = threshold.fracture), 1, 0)
# confusion matrix
t3 <- table(data.q3.men.test$fracture,preds3)
# create vector of misclassification rate
temp.err3<- (t3[1,2]+t3[2,1])/sum(t3) ## cannot calcualte the error because only 0 in prediction
glmnet.err.q3.men<- rbind(glmnet.err.q3.men,temp.err3)

# find BMD
x.men <- scale(model.matrix(fracture~., data.q3.men)[,-1])
y.men <- data.q3.men$fracture
set.seed(1234)
...
BMDfeature.men
## "DXXWDBMD" "DXXOSBMD" "DXXL1BMD" "DXXL3BMD"

## women
data.q3.women<-frax_risk_women[,!names(frax_risk_women) %in% c("RIAGENDR","OSQ010A","OSQ010B","OSQ010C")]
train.women<-train%>%filter(RIAGENDR==2)
test.women<-test%>%filter(RIAGENDR==2)
data.q3.women.train<-train.women[,!names(train.women) %in% c("RIAGENDR","OSQ010A","OSQ010B","OSQ010C")]
data.q3.women.test<-test.women[,!names(test.women) %in% c("RIAGENDR","OSQ010A","OSQ010B","OSQ010C")]
glmnet.err.q3.women <- NULL
# find the error
set.seed(1234)
x4.tr <-scale(model.matrix(fracture~., data.q3.women.train)[,-1])
y4.tr <- data.q3.women.train$fracture
cv.l4.tr <- cv.glmnet(x4.tr,y4.tr,family="binomial",nfolds = 5,alpha=1,penalty.factor =c(rep(0,5),rep(1,10),rep(0,12)))
best_lambda.l4<-cv.l4.tr$lambda.min
x4.test<-scale(model.matrix(fracture~., data.q3.women.test)[,-1])
prob4<-predict(cv.l4.tr,x4.test,s=best_lambda.l3,type="response")
preds4<- ifelse(as.numeric(prob4) > quantile(prob4,probs = threshold.fracture), 1, 0)
# confusion matrix
t4 <- table(data.q3.women.test$fracture,preds4)
# create vector of misclassification rate, specificity, sensitivity and precision
temp.err4<- (t4[1,2]+t4[2,1])/sum(t4)
glmnet.err.q3.women<- rbind(glmnet.err.q3.women,temp.err4)

# find BMD
x.women <-scale(model.matrix(fracture~.,data.q3.women)[,-1])
...
library(gbm)
###### Q1: identify predictors (BMD) of osteoporotic fracture in whole cohort, men and women
# Step 1: prepare features for modeling (in crystal's code)
# Get the imputed values
impute <- impute.transcan(impute_arg, data=df, imputation=1, list.out=TRUE, pr=FALSE, check=FALSE)
impute <- as.data.frame(do.call(cbind,impute))
impute<-impute[,-c(4:6)]
...
frax_risk_men<-impute%>%filter(RIAGENDR==1) %>% select(-c(all_of(fracture_variables),all_of(individual_variables)))
frax_risk_women<-impute%>%filter(RIAGENDR==2)%>% select(-c(all_of(fracture_variables),all_of(individual_variables)))
...
xgbmale <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_men)),ncol = ncol(frax_risk_men), byrow = FALSE ) ,label = impute$fracture[which(impute$RIAGENDR==1)]) 
xgbfemale <- xgb.DMatrix(data = matrix(as.numeric(unlist(frax_risk_women)),ncol = ncol(frax_risk_women), byrow = FALSE ) ,label = impute$fracture[which(impute$RIAGENDR==2)]) 
...
for(i in seq_len(nrow(hyper_grid))) {
  ...
  m2 <- xgb.train(data = xgbmale,nrounds = 2,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],
                    eta = hyper_grid$eta[i],min_child_weight = hyper_grid$min_child_weight[i], 
      subsample=.8, colsample_bytree=1)
 m3 <- xgb.train(data = xgbfemale,nrounds = 2,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],
                    eta = hyper_grid$eta[i],min_child_weight = hyper_grid$min_child_weight[i], 
      subsample=.8, colsample_bytree=1)
  # add SSE, trees, and training time to results
  pred_male<- predict(m2, xgbmale)
  pred_female <- predict(m3, xgbfemale)
  pred_male <- as.numeric(pred_male > sort( pred_male, decreasing = T)[sum(impute$fracture[which(impute$RIAGENDR==1)])])
  pred_female <- as.numeric(pred_female > sort( pred_female, decreasing = T)[sum(impute$fracture[which(impute$RIAGENDR==2)])])
  hyper_grid$accuracy_male[i]  <- mean(pred_male == impute$fracture[which(impute$RIAGENDR==1)])
  hyper_grid$accuracy_female[i]  <- mean(pred_female == impute$fracture[which(impute$RIAGENDR==2)])
}
arrange(hyper_grid,accuracy_male);arrange(hyper_grid,accuracy_female)
bst_male <- xgb.train(data = xgbmale,nrounds = 2,objective = "binary:logistic",
      max.depth = 4,eta = 0.2,min_child_weight = 8, subsample=.7, colsample_bytree=1)
bst_female <- xgb.train(data = xgbfemale,nrounds = 2,objective = "binary:logistic",
      max.depth = 4,eta = 0.2,min_child_weight = 6, subsample=.7, colsample_bytree=1)
...
xgbhip<- xgb.DMatrix(data = matrix(as.numeric(unlist(x.risk)),ncol = ncol(x.risk), byrow = FALSE ) ,label =  y.hip) 
xgbfrac<- xgb.DMatrix(data = matrix(as.numeric(unlist(x.risk)),ncol = ncol(x.risk), byrow = FALSE ) ,label =  y.frac) 
hyper_grid <- expand.grid(
  booster = "gbtree",
  max.depth = c(4,6,8),
...
for(i in seq_len(nrow(hyper_grid))) {
  set.seed(123) # for reproducibility
  m1 <- xgb.train(data = xgbhip,nrounds = 3,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],
                    eta = hyper_grid$eta[i],min_child_weight = hyper_grid$min_child_weight[i], 
  subsample=.7, colsample_bytree=1)
  m2 <- xgb.train(data = xgbfrac,nrounds = 3,objective ="binary:logistic",max.depth=hyper_grid$max.depth[i],
                    eta = hyper_grid$eta[i],min_child_weight = hyper_grid$min_child_weight[i], 
  subsample=.7, colsample_bytree=1)
  pred_hip<- predict(m1, xgbhip)
  pred_frac<- predict(m2, xgbfrac)
  pred_hip<- as.numeric(pred_hip > sort( pred_hip, decreasing = T)[sum(y.hip)])
  pred_frac<- as.numeric(pred_frac > sort( pred_frac, decreasing = T)[sum(y.frac)])
  hyper_grid$accuracy_hip[i]  <- mean(pred_hip == y.hip)
  hyper_grid$accuracy_whole[i]  <- mean(pred_frac == y.frac)
}
arrange(hyper_grid, accuracy_whole)
bst_hip <- xgb.train(data = xgbhip,nrounds = 2,objective ="binary:logistic",max.depth=6,
                    eta = 0.1,min_child_weight =6, subsample=.7, colsample_bytree=1)
bst_whole <- xgb.train(data = xgbfrac,nrounds = 2,objective ="binary:logistic",max.depth=8,
                    eta = 0.2,min_child_weight =6, subsample=.7, colsample_bytree=1)
xgb.plot.importance(xgb.importance(names(x.hip),model = bst_whole))
######## DXXTRBMD - Trochanter BMD measure is the best predictor of osteoporotic  fracture
xgb.plot.importance(xgb.importance(names(x.hip),model = bst_hip))
