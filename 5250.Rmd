...
bayesian.f <- function(outcome.var, d, set.seed, features){
  ...
  set.seed(set.seed)
  # 1. baseline model
  fit <- post.f(outcome.var, d, set.seed, features)
  post_null <- fit$post
  elpd_null <- fit$elpd
  while(k>0){
    post <- NULL
    ...
    for (i in 1:length(features)){
      fit <- post.f(outcome.var, d, set.seed, features[-i])
      post[[i]] <- fit$post
      elpd[i] <- fit$elpd
    }
...
# selected features
BMD <- whole.fracture$BMD#DXXL3BMD
features <- whole.fracture$features
#"OSQ130"   "OSQ170"   "OSQ200"   "SMQ020"   "ALQ101"   "MCQ160A"  "MCQ160L"  "RIDRETH1"
# performance
d <- test.b
auc <- performance.f(whole.fracture, d)$auc# Area under the curve: 0.6355
accurary <- performance.f(whole.fracture, d)$accurary# accurary :0.61
...
auc <- performance.f(women.fracture, d)$auc#Area under the curve: 0.6375
accurary <- performance.f(women.fracture, d)$accurary#0.64
...
d <- d %>% 
  mutate(OSQ010A=as.factor(ifelse(OSQ010A=="1", 1, 0))) %>%
  select(-fracture)
auc <- performance.f(whole.hip, d)$auc#Area under the curve: 0.6691
