...
probs.full <- attr(predict(svmfit, data1.test,probability=TRUE),"probabilities")[,"0"]
...
data2.train <- data2[train_ind,] # set train dataset for question 2
...
# * assess the test error using tuned kernel type on predefined test set ----
svmfit.hip = svm(OSQ010A~.,kernel=type[which.min(cv.hip)], 
             data=data2.train,cost=1,probability = TRUE)
# get predicted probs for class 2
probs.hip <- attr(predict(svmfit.hip, data2.test,probability=TRUE),
                  "probabilities")[,"2"]
thresh.hip <- mean(data2.test$OSQ010A==2)
preds.hip <- ifelse(probs.hip>quantile(probs.hip,probs=thresh.hip),1,0)
auc(data2.test$OSQ010A, as.numeric(preds.hip)) #0.5056
mean(as.numeric(preds.full)==data1.test$fracture) # test accuracy 78.42%
